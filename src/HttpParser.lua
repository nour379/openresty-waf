...
local waf_controller = require "waf_controller"

local table_concat = table.concat
local table_insert = table.insert

_M.version = version.version

function _M.parse_request(request_headers, collections)
  ...
  if ngx.re.find(content_type_header, [=[^multipart/form-data; boundary=]=], waf_controller._pcre_flags) then
    if not waf_controller._process_multipart_body then
      return
    end

    local form, err = upload:new()
    if not form then
      logger.warn(waf_controller, "failed to parse multipart request: ", err)
    ...
    return nil
  elseif ngx.re.find(content_type_header, [=[^application/x-www-form-urlencoded]=], waf_controller._pcre_flags) then
  ...
  elseif util.table_has_key(content_type_header, waf_controller._allowed_content_types) then
    ...
    if waf_controller._allow_unknown_content_types then
